...
public class RobotContainer {
  ...
  SwerveInputStream driveAngularVelocityKeyboard = SwerveInputStream
      ...
      .deadband(OperatorConstants.DEADBAND).scaleTranslation(0.9).allianceRelativeControl(true);
  ...
  private void configureBindings() {
    ...
    driverXbox.x().onTrue(targetingSubsystem.autoTargetPairCommand(drivebase::getPose, Side.LEFT)
        .andThen(Commands.either(targetingSubsystem.driveToCoralTarget(drivebase, 0.6, 1),
            Commands.runEnd(() -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 1),
                () -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 0)).withTimeout(.15),
            () -> targetingSubsystem.areWeAllowedToDrive(drivebase::getPose))));

    driverXbox.b().onTrue(targetingSubsystem.autoTargetPairCommand(drivebase::getPose, Side.RIGHT)
        .andThen(Commands.either(targetingSubsystem.driveToCoralTarget(drivebase, 0.6, 1),
            Commands.runEnd(() -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 1),
                () -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 0)).withTimeout(.15),
            () -> targetingSubsystem.areWeAllowedToDrive(drivebase::getPose))));
    ...
    driverXbox.a().onTrue(Commands.runOnce(drivebase::zeroGyro).andThen(Commands
        .print(DriverStation.isTest() ? "Test Mode: Reset Gyro" : "Other Mode: Reset Gyro")));
    ...
    driverXbox.leftBumper().whileTrue(Commands.either(climberSubsystem.playMusicCommand(),
        Commands.runOnce(drivebase::lock, drivebase).repeatedly(), DriverStation::isTest));

    driverXbox.rightBumper().and(DriverStation::isTest).onTrue(scoringSubsystem.runIntakeCommand());

    driverXbox.rightTrigger().and(DriverStation::isTeleop)
    ...
    m_JoystickL.trigger().onTrue(Commands.either(scoringSubsystem.runIntakeCommand(),
        scoringSubsystem.runEjectCommand(), scoringSubsystem::getCoralSensor));
    ...
    m_JoystickL.trigger().and(DriverStation::isTest)
        .whileTrue(elevator.setSpeed(() -> m_JoystickL.getY() * -1));

    m_JoystickL.button(2).and(() -> !DriverStation.isTest())
        .whileTrue(elevator.setSpeed(() -> m_JoystickL.getY() * -1));
    ...
    m_JoystickL.button(7)
        .toggleOnTrue(
            Commands
                .either(
                    Commands.repeatingSequence(
                        elevator.setPos(() -> FieldConstants.CoralStation.height).alongWith(
                            scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch)),
                        Commands.waitSeconds(2),
                        elevator.setPos(() -> FieldConstants.ReefHeight.L2.height).alongWith(
                            scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L2.pitch)),
                        Commands.waitSeconds(2),
                        elevator.setPos(() -> FieldConstants.ReefHeight.L3.height).alongWith(
                            scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L3.pitch)),
                        Commands.waitSeconds(2),
                        elevator.setPos(() -> FieldConstants.ReefHeight.L4.height)
                            .alongWith(
                                scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L4.pitch))
                            .andThen(scoringSubsystem.tiltCommand(144)),
                        Commands.waitSeconds(2)),
                    Commands.none(), DriverStation::isTest));

    /**
     * ------------------------------------- Climber bindings --- climb and descend ---
     */

    m_JoystickL.button(9)
        .whileTrue(climberSubsystem.descendCommand().alongWith(Commands.print("climber down")));
    m_JoystickL.button(10)
    ...
    m_JoystickL.button(2).onTrue(Commands.either(scoringSubsystem.tiltSysIDCommand(4, 2, 2),
        Commands.none(), DriverStation::isTest));

    m_JoystickR.button(9).whileTrue(Commands.either(elevatorSubsystem.sysIDCommand(4, 2, 2),
        Commands.none(), DriverStation::isTest));

    m_JoystickR.button(10).whileTrue(Commands.either(scoringSubsystem.tiltSysIDCommand(4, 2, 2),
        Commands.none(), DriverStation::isTest));
    m_JoystickR.button(11).whileTrue(Commands.either(drivebase.sysIdAngleMotorCommand(),
        Commands.none(), DriverStation::isTest));
    m_JoystickR.button(12).whileTrue(Commands.either(drivebase.sysIdDriveMotorCommand(),
        Commands.none(), DriverStation::isTest));
  }
  ...
  public Command score =
      Commands.waitUntil(() -> scoringSubsystem.atDesiredPosition() && elevator.atDesiredPosition())
          .withTimeout(1.5).andThen(scoringSubsystem.runEjectCommand());
  public Command intake = scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch)
      .andThen(Commands
          .waitUntil(() -> scoringSubsystem.atDesiredPosition() & elevator.atDesiredPosition())
          .withTimeout(1.5).andThen(scoringSubsystem.runIntakeCommand())
          .andThen(new PrintCommand("Coral sensor active, running intake")
  ...
  public Command L1 = elevator.setPos(() -> FieldConstants.CoralStation.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch))
      .until(elevator::atDesiredPosition);
  public Command L2 = elevator.setPos(() -> FieldConstants.ReefHeight.L2.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch))
      .until(elevator::atDesiredPosition)
      .andThen(scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L2.pitch))
      .until(scoringSubsystem::atDesiredPosition);
  public Command L3 = elevator.setPos(() -> FieldConstants.ReefHeight.L3.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch))
      .until(elevator::atDesiredPosition)
      .andThen(scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L3.pitch))
      .until(scoringSubsystem::atDesiredPosition);
  public Command L4 = elevator.setPos(() -> FieldConstants.ReefHeight.L4.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch))
      .until(elevator::atDesiredPosition)
  ...
  public Command autoTargetLeftBranchCommand =
      targetingSubsystem.autoTargetPairCommand(drivebase::getPose, Side.LEFT)
          .andThen(Commands.either(targetingSubsystem.driveToCoralTarget(drivebase),
              Commands.runEnd(() -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 1),
                  () -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 0)).withTimeout(.2),
              () -> targetingSubsystem.areWeAllowedToDrive(drivebase::getPose)))
          .withName("autoTargetLeftBranch");

  public Command autoTargetRightBranchCommand =
      targetingSubsystem.autoTargetPairCommand(drivebase::getPose, Side.RIGHT)
          .andThen(Commands.either(targetingSubsystem.driveToCoralTarget(drivebase),
              Commands.runEnd(() -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 1),
                  () -> driverXbox.getHID().setRumble(RumbleType.kBothRumble, 0)).withTimeout(.2),
              () -> targetingSubsystem.areWeAllowedToDrive(drivebase::getPose)))
  ...
  public Command autoLvl4Command = elevator.setPos(() -> FieldConstants.ReefHeight.L4.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L4.pitch))
      .until(elevator::atDesiredPosition).andThen(scoringSubsystem.runIntakeCommand())
      .withName("autoLvl4");

  public Command autoLvl3Command = elevator.setPos(() -> FieldConstants.ReefHeight.L3.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L3.pitch))
      .until(elevator::atDesiredPosition).andThen(scoringSubsystem.runIntakeCommand())
      .withName("autoLvl3");

  public Command autoLvl2Command = elevator.setPos(() -> FieldConstants.ReefHeight.L2.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.ReefHeight.L2.pitch))
      .until(elevator::atDesiredPosition).andThen(scoringSubsystem.runEjectCommand())
      .withName("autoLvl2");

  public Command autoIntakeCommand = elevator.setPos(() -> FieldConstants.CoralStation.height)
      .alongWith(scoringSubsystem.tiltCommand(FieldConstants.CoralStation.pitch))
      .until(elevator::atDesiredPosition).andThen(scoringSubsystem.runIntakeCommand())
      .withName("autoIntake");

  public Command autoAlignSourceCommand = targetingSubsystem.driveToSourceCommand(drivebase)
      .alongWith(autoIntakeCommand).withName("autoAlignSource");
