...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    swerveDrive.setAngularVelocityCompensation(
        true, true, 0.1); // Correct for skew that gets worse as angular velocity
    // increases. Start with a
    // coefficient of 0.1.
    swerveDrive.setModuleEncoderAutoSynchronize(
        false, 1); // Enable if you want to resynchronize your absolute encoders
    // anti jitter
              ...
              drive(
                  getTargetSpeeds(
                      ...
                      Rotation2d.fromDegrees(
                          result
                              .getBestTarget()
                              .getYaw()))); // Not sure if this will work, more math
            }
          }
        });
  }
  ...
  public Command sysIdDriveMotorCommand() {
    return SwerveDriveTest.generateSysIdCommand(
        SwerveDriveTest.setDriveSysIdRoutine(
            new Config(null, Voltage.ofBaseUnits(9, Volts), null, null),
            this,
            swerveDrive,
            9,
            false),
  ...
  public Command driveToDistanceCommand(double distanceInMeters, double speedInMetersPerSecond) {
    return new Command() {
      ...
      public boolean isFinished() {
        return (swerveDrive.getPose().getTranslation().getDistance(startTranslation)
            > distanceInMeters);
      }
    };
    // return run(() -> {
  ...
  public Command robotDriveCommand(
      Supplier<ChassisSpeeds> velocity, BooleanSupplier robotRelative) {
    return run(
        () -> {
          if (robotRelative.getAsBoolean()) {
            swerveDrive.driveFieldOrientedAndRobotOriented(
                new ChassisSpeeds(0, 0, 0), velocity.get());
          } else {
            swerveDrive.driveFieldOriented(velocity.get());
          }
        });
