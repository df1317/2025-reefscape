...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public ElevatorSubsystem() {
    ...
    motorL.configure(
        config.inverted(false), ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
    motorR.configure(
        config.inverted(true), ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
  ...
  public void periodic() {
    ...
    if (running) {
      controllerL.setReference(
          preRenfernce.position * krot, ControlType.kPosition, ClosedLoopSlot.kSlot0, ffValue);
                ...
                log.motor("elevator")
                    .voltage(
                        m_appliedVoltage.mut_replace(
                            motorL.getBusVoltage() * motorL.getAppliedOutput(), Volts))
                    .linearPosition(m_distance.mut_replace(encoderL.getPosition() / krot, Meters))
                    .linearVelocity(
                        m_velocity.mut_replace(
                            encoderL.getVelocity() / 60.0 / krot, MetersPerSecond));
              },
