...
public class TargetingSubsystem extends SubsystemBase {
  ...
  public TargetingSubsystem() {
    new Trigger(() -> DriverStation.getAlliance().isPresent())
        .toggleOnTrue(Commands.runOnce(this::initializeBranchPoses));
  }
  ...
  public Command driveToCoralTarget(
      SwerveSubsystem swerveDrive, double speed, double acceleration) {
  ...
  public Pose2d getCoralTargetPose() {
    ...
    if (targetBranch != null) {
      Pose2d startingPose =
          Reef.branchPositions.get(targetBranch.ordinal()).get(ReefHeight.L2).toPose2d();
      SmartDashboard.putString(
          "Targetted Coral Pose without Offset (Meters)", startingPose.toString());
      scoringPose =
          startingPose.plus(
              targetSide == Side.LEFT
                  ? AutoScoring.Reef.coralOffsetL
                  : AutoScoring.Reef.coralOffsetR);
  ...
  public Pose2d autoTargetPair(Supplier<Pose2d> currentPose, Side preferredSide) {
    ...
    ReefBranch pairBase =
        ReefBranch.values()[closestBranch.ordinal() - (closestBranch.ordinal() % 2)];
