...
public class ScoringSubsystem extends SubsystemBase {
  ...
  public ScoringSubsystem() {
    ...
    motor2.configure(
        motorConfig2, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
    ...
    tiltConfig
        .closedLoop
        .p(4.5, ClosedLoopSlot.kSlot0)
        .i(0, ClosedLoopSlot.kSlot0)
        .d(0.4, ClosedLoopSlot.kSlot0);
    canTiltMax = new SparkMax(CanConstants.scoreTiltMotor, MotorType.kBrushed);
    canTiltMax.configure(
        tiltConfig, ResetMode.kResetSafeParameters, PersistMode.kNoPersistParameters);
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "scoring/tilt voltage", canTiltMax.getBusVoltage() * canTiltMax.getAppliedOutput());
    ...
    SmartDashboard.putNumber(
        "scoring/motor1 voltage", motor1.getBusVoltage() * motor1.getAppliedOutput());
    SmartDashboard.putNumber("scoring/motor2 current", motor2.getOutputCurrent());
    SmartDashboard.putNumber(
        "scoring/motor2 voltage", motor2.getBusVoltage() * motor2.getAppliedOutput());
  ...
  public Command tiltSysIDCommand(double quasiTimeout, double timeout, double dynamicTimeout) {
    return m_sysIdRoutine
        ...
        .andThen(
            m_sysIdRoutine.quasistatic(SysIdRoutine.Direction.kReverse).withTimeout(quasiTimeout))
        .andThen(Commands.waitSeconds(timeout))
        .andThen(
            m_sysIdRoutine.dynamic(SysIdRoutine.Direction.kForward).withTimeout(dynamicTimeout))
        .andThen(Commands.waitSeconds(timeout))
        .andThen(
            m_sysIdRoutine.dynamic(SysIdRoutine.Direction.kReverse).withTimeout(dynamicTimeout));
  }
                ...
                log.motor("tilt")
                    .voltage(
                        m_appliedVoltage.mut_replace(
                            canTiltMax.getBusVoltage() * canTiltMax.getAppliedOutput(), Volts))
                    .angularPosition(
                        m_rotation.mut_replace(canTiltEncoder.getPosition(), Rotations))
                    .angularVelocity(
                        m_velocity.mut_replace(canTiltEncoder.getVelocity(), RotationsPerSecond));
              },
